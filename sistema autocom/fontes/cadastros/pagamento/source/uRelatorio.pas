...
begin
  ...
  while not DsRel.Eof do
    begin
      ...
      if DsRel.FieldByName('AUTENTICA').AsString = 'T' then Dm.CdsCPAUTENTICA.Value := 'Sim' else Dm.CdsCPAUTENTICA.Value := 'Não';
      if DsRel.FieldByName('IMPRESSAOCHEQUE').AsString = 'T' then Dm.CdsCPIMPRESSAOCHEQUE.Value := 'Sim' else Dm.CdsCPIMPRESSAOCHEQUE.Value := 'Não';

      case DsRel.FieldByName('TIPOTROCO').AsInteger of
        0: Dm.CdsCPTIPOTROCO.AsString := 'NÃO PERMITE';
      ...
      case DsRel.FieldByName('FUNCAOESPECIAL').AsInteger of
        0: Dm.CdsCPTIPOTROCO.AsString := 'SEM FUNÇÃO';
        1: Dm.CdsCPTIPOTROCO.AsString := 'CONSULTA LISTA NEGRA';
        2: Dm.CdsCPTIPOTROCO.AsString := 'CONVÊNIO';
      ...
      if DsRel.FieldByName('ATIVOVENDA').AsString = 'T' then Dm.CdsCPATIVOVENDA.Value := 'Sim' else Dm.CdsCPATIVOVENDA.Value := 'Não';
      if DsRel.FieldByName('ATIVOFINANCEIRO').AsString = 'T' then Dm.CdsCPATIVOFINANCEIRO.Value := 'Sim' else Dm.CdsCPATIVOFINANCEIRO.Value := 'Não';
      if DsRel.FieldByName('SOMASALDO').AsString = 'T' then Dm.CdsCPSOMASALDO.Value := 'Sim' else Dm.CdsCPSOMASALDO.Value := 'Não';
...
begin
  if DsRel.FieldByName('ATIVOVENDA').AsString = 'T' then DsRel.FieldByName('BAtivoVenda').Value := 'Sim' else DsRel.FieldByName('BAtivoVenda').Value := 'Não';
  if DsRel.FieldByName('ATIVOFINACEIRO').AsString = 'T' then DsRel.FieldByName('BAtivoFinanceiro').Value := 'Sim' else DsRel.FieldByName('BAtivoFinanceiro').Value := 'Não';
  if DsRel.FieldByName('IMPRESSAOCHEQUE').AsString = 'T' then DsRel.FieldByName('BCheque').Value := 'Sim' else DsRel.FieldByName('BCheque').Value := 'Não';
  if DsRel.FieldByName('ATENTICA').AsString = 'T' then DsRel.FieldByName('BAtentificar').Value := 'Sim' else DsRel.FieldByName('BAtentificar').Value := 'Não';
  if DsRel.FieldByName('SOMASALDO').AsString = 'T' then DsRel.FieldByName('BSomaSaldo').Value := 'Sim' else DsRel.FieldByName('BSomaSaldo').Value := 'Não';
end;

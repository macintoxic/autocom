...
const
     ...
     Nome_sistema_cabecalho='AutocomPLUS - Cardápio - Delivery PDV';
...
type
  Tfrm_FATCPR = class(TForm)
    ...
    procedure mi207Click(Sender: TObject);
...
procedure Tfrm_FATCPR.mi207Click(Sender: TObject);
var
   Menu:TsuiImageButton;
   s_MenuNome:string;
begin
...
procedure Tfrm_FATCPR.FormKeyDown(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
     if (key=vk_f1) and (mi207.Enabled=true)          then mi207Click(mi207);
     if (key=vk_f2) and (mi205.Enabled=true)          then mi207Click(mi205);
     if (key=vk_f3) and (mi801.Enabled=true)          then mi207Click(mi801);
     if (key=vk_f4) and (mi802.Enabled=true)          then mi207Click(mi802);
     if (key=vk_f5) and (mi109.Enabled=true)          then mi207Click(mi109);

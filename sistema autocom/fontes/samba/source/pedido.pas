      ...
      begin
       ...
       begin
         ...
         if dtm2.produtos.isempty then
           begin
             label1.caption:='Código Inválido';
             e2produto.setfocus;
             e2produto.clear;
             Exit;
           end
...
if (dtm2.produtos.fields[0].asfloat <> 0) and (not dtm2.produtos.fields[0].IsNull) then
  ...
  begin
    MessageBox(Handle, 'Esse produto não pode ser vendido, pois não há preço cadastrado', 'Erro', MB_ICONWARNING);
    label1.Caption := '';
    e2produto.setfocus;
    e2produto.Clear;
  end;
     ...
     if (StrToFloat(Mesa) >= I_LimitePedidoMinimo)and (StrToFloat(Mesa) <= I_LimitePedidoMaximo) then
      ...
      {Se o campo Situacao estiver com x na tabela PEDIDOVENDA, acesso negado!}
          if (Dtm2.produtos.FieldByName('SITUACAO').AsString <> 'X') then
             ...
             begin
                messagedlg('Esta '+ S_NomePedido +' já foi fechada' ,mtinformation,[mbok],0);
                e1mesa.setfocus;
                e1mesa.Clear;
             end;
        end
...
if (local = 'Quantidade') and (not IsNull(e1qtde.Text)) Then
   begin
     ...
     if B_obs = true then
       begin
         if (application.MessageBox('Deseja Incluir Obervação?','Autocom - Samba',MB_YESNO) = IDYes) then
            begin
               fobs.showmodal;
            end
         else

...
begin
    case key of
        VK_F1: spdFLX.Click;
        VK_F2: spdCancCupon.Click;
        VK_F3: spdZ.Click;
        VK_F4: spdFundo.Click;
        VK_F5: spdSangria.Click;
        VK_F6: spdConsultaProduto.Click;
        VK_F7: spdAbreGaveta.Click;
        VK_F8: spdTransferencia.Click;
        VK_F12: spdSair.Click;
        VK_ESCAPE: spdVolta.Click;
    end;
end;
    ...
    begin
        ...
        if straux[1] = '#' then
            Application.MessageBox(Pchar('Ocorreu um erro durante o envio do comando para a impressora'#13 +
                ...
                MB_ICONERROR + MB_OK);
        if Application.MessageBox('Enviar formas de pagamento para a impressora?',TITULO_MENSAGEM, MB_YESNO + MB_ICONQUESTION ) = IDYES then
        begin
             @InicioDia := GetProcAddress(hand, 'InicioDia');
            straux := InicioDia(StrToInt(leini('terminal', 'ModECF')),
                    StrToInt(leini('terminal', 'comECF')),
                    LeINI('terminal', 'hv'), LeINI('oper', 'codigo',
                        'dados\oper.ini'),

...
type
    ...
    lblIndicador: TLabel;
    cdsItensPedidoLote: TStringField;
    pnlqtdeunt: TPanel;
    Panel2: TPanel;
...
begin
    ...
    case straux[6] of
        ...
        '3':
            begin
                 if TrataerroImpressora(FFinalizadia(GetProcAddress(hand, 'Finalizadia'))(StrToInt(leini('terminal', 'ModECF')),
                    StrToInt(leini('terminal', 'comECF'))),True) then
                 begin
                      Application.MessageBox('Erro no fechamento do dia. Finalizando',TITULO_MENSAGEM,MB_ICONERROR or MB_OK);
                      PostMessage(Handle, WM_SYSCOMMAND, SC_CLOSE, SC_CLOSE);
                 end;
...
//        bDireto := True;
        ...
        begin
            ...
            actGravarExecute(actFechamento); //grava o pedido

{
            ...
            begin
                ...
                cdsItensPedidoLote.Value :=
                    auxdataset.FieldByName('LOTE').AsInteger;
...
begin
    ...
    begin
        ...
        // POr Helder Frederico dia 05/11/2003 - Verifica se o produto controla lote
        if prdProduto.controlalote = '1' then
           begin
              label2.enabled:=true;
              edlote.enabled:=true;
              edlote.setfocus;
...
begin
    ...
    begin
        ...
        // dia 05/04/2003 por Helder Frederico - Verifica se cadastrou informou a edicao
        if edlote.enabled=true then
           begin
              try
                 strtofloat(edlote.text);
              except
                 Application.MessageBox('Edição não informada. Verifique!', TITULO_MENSAGEM, MB_OK + MB_ICONERROR);
                 edlote.setfocus;
                 Exit;
              end;
           end;
        ...
        begin
            ...
            log('preco encontrado ' + auxprod.preco);
            cdsItensPedidoPedido.AsString := mskControle.Text;
            cdsItensPedidocodigo.Value := auxProd.codigo;
            cdsItensPedidoDescricao.Value := auxProd.descricao;
            cdsItensPedidoValorUnitario.AsString := ifthen(bLancouMeio, floattostr(preco), auxprod.preco);
            cdsItensPedidoCasasDecimais.Value := auxProd.decimais;
            cdsItensPedidoQuantidade.Value := StrToFloat(mskQTDE.Text);
            try // dia 04/11/2003 por Helder Frederico - informa grava o lote (edicao) do produto!
               strtofloat(edlote.text);
               cdsItensPedidoLote.value := edlote.text;
            except
               cdsItensPedidoLote.value :='0';
            end;

            log('Inserindo no XML - Produto:'+cdsItensPedidocodigo.asstring+'Lote: '+cdsItensPedidoLote.value);
...
begin
    ...
    begin
        if dbgrdItens.SelectedRows.Count = 0 then
            ...
            begin
                if Application.MessageBox(PChar('Confirma o cancelamento deste item?'#13 +
                    'Produto : '+cdsItensPedidoDescricao.Value),TITULO_MENSAGEM,MB_YESNO + MB_ICONQUESTION) = idyes then
...
//                auxTotalItem :=dbgrdItens.DataSource.DataSet.FieldByName('total').AsFloat;
                ...
                //dia 06/11/2003 por Helder Frederico: Devido a correcao do charles (beforeedit),
                // estava retirando o valor do item 2x, por isso desabilitei o subtracao abaixo
...
begin
    ...
    begin
        aux := 0;
        // dia 06/11/2003 por Helder Frederico: estava forçando o codigocliente=1
        DMORC.RunSQL('SELECT CLI_CODCLIENTE FROM CLIENTE WHERE CODIGOCLIENTE = '+mskcliente.text,
            auxdataset);
        ...
        begin
            ...
            dmorc.InserePedido
            (cdsItensPedido, //items do pedido
                ...
                auxdataset.Fields[0].asstring, // codigo do cliente
            ...
            if strtointdef(leini('terminal', 'printgrill'), 0) = 1 then
                printgrill2(StrToInt(mskControle.Text), leini(TIPO_TERMINAL,'legenda'),mskIndicador.Text);
...
//        auxDataSet := dmORC.ConsultaPedido(mskControle.Text);
        ...
        begin
            ...
            try // dia 04/11/2003 por Helder Frederico - informa grava o lote (edicao) do produto!
               if length(trim(auxDataSet.FieldByName('LOTE').asstring))>0 then
               cdsItensPedidoLote.value := auxDataSet.FieldByName('LOTE').asstring
               else
               cdsItensPedidoLote.value :='0';
            except
               cdsItensPedidoLote.value :='0';
...
begin
    ...
    lblDescricao.Caption := '';
    lblPrecoUnitario.Caption := '0' + DecimalSeparator + '00';
    // dia 06/11/2003 por Helder Frederico: desabilita os componentes de controle de edicao
    label2.enabled:=false;
    edlote.enabled:=false;
...
procedure TfrmPDV.cdsItensPedidoBeforePost(DataSet: TDataSet);
begin
     log('Post preco ->>' + dataset.fieldbyname('valor unitario').AsString)
end;

procedure TfrmPDV.cdsItensPedidoBeforeEdit(DataSet: TDataSet);
begin
    //estava mostrando o total incorretamente.
    //Corrigir nos outros módulos tbm.
    TotalProdutos :=  TotalProdutos - DataSet.FieldByName('Total').AsFloat;
end;

procedure TfrmPDV.edloteEnter(Sender: TObject);
begin
     edlote.clear;
end;

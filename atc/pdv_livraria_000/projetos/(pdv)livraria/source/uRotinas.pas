...
procedure GravaFechamentoCupom(CondicaoPagamento, repique, troco,
    ...
    agencia, conta, datapre, vendedor, NCP: string);
...
const
    ...
    TIPO_TERMINAL = 'LIVRARIA';
    //ALTERAR DE ACORDO COM O TERMINAL. VALORES :  RESTAURANTE, DELIVERY, CAIXA, LIVRARIA
...
var
    ...
    //06/11/2003 por Helder Frederico: variaveis de controle de lancamento de descontos
    // e acrescimo. OBS: O Charles Odeia isso!! heheh
    lancou_acrescimo :real;
    lancou_desconto :real;
...
begin
    ...
    straux := AbreCupom(StrToInt(leini('terminal', 'ModECF')), StrToInt(leini('terminal', 'comECF')), '');
...
//    straux := Copy(straux, 2, 6);
    COO := Copy(straux, 2, 6);
    GravaINI(TIPO_TERMINAL, 'ultimocupom', COO);

// 06/11/2003 por Helder Frederico: Coloquei no GRavaCabecalhoCupom
...
//        auxdataset.FieldByName('totalpedido').AsString);
    ...
    if bCabecalhoCupom then
        GravaCabecalhoCupom(NumeroPedido, COO);

    FreeLibrary(hand);

    //laça os item no ecf e grava no banco de dados
    conta_item:=1;
    ...
    begin
        ...
        if cdsItems.FieldByName('situacao').AsInteger = 0 then
            FLancaitem(GetProcAddress(hand, 'Lancaitem'))(
                StrToInt(leini('terminal', 'ModECF')),
                StrToInt(leini('terminal', 'comECF')),
                cdsItems.FieldByName('CODIGO').AsString,
        ...
        // DONE : Não esquecer de arrumar a tributação. Por enquanto está chumbada.
            ...
            //dia 06/11/2003 por Helder Frederico: Coloquei a grvacao do produto junto com a impressao
            DecimalSeparator := '.';
            dmORC.RunSQL('INSERT INTO PDV_DETALHECUPOM ' +
                ...
                'SEQ, VALORUN, QTDE, DESCONTO, ACRESCIMO,SITUACAO, LOTE) ' +
                'VALUES ' +
                '(0, 1,1, ' +
                Vendedor + ', ' +
                ...
                inttostr(conta_item) + ', ' +
                cdsItems.FieldByName('Valor Unitario').AsString + ', ' +
                cdsItems.FieldByName('QUANTIDADE').AsString + ', ' +
                '0, ' +
                '0, ' +
                cdsItems.FieldByName('situacao').AsString +', '+
                quotedstr(cdsItems.FieldByName('Lote').AsString)+') ',auxdataset);
            DecimalSeparator := ',';
        cdsItems.Next;
    ...
    if lancou_desconto > 0 then
        ajustaacrescimo(floattostr(lancou_desconto), numeropedido, 3);
    if Lancou_acrescimo > 0 then
        ajustaacrescimo(floattostr(lancou_acrescimo), numeropedido, 1);


 //   gravadetalhecupom(NumeroPedido, coo);


    // Faltava lançar Item de servico no cupom fiscal - 25/04/2003 - Charles
    // dia 06/11/2003 - por Helder Frederico: troque o if abaixo pelo lancou_acrescimo
//    if auxdataset.FieldByName('DespesasAcessorias').AsFloat > 0 then
    ...
    begin
        Lancaitem(StrToInt(leini('terminal', 'ModECF')),
            ...
            floattostr(lancou_acrescimo),
            floattostr(lancou_acrescimo), 'I  ');
        // servico é isento.
    end;
    FreeLibrary(hand);
end;
...
function TrataerroImpressora(strResposta: string; bShow: boolean = true):
    boolean;
begin
    log(ifthen(strResposta[1] = '@', 'ECF OK -> ' + strResposta, 'ECF -> ERRO: '));
...
begin
    ...
    //06/11/2003 - por Helder Frederico: Coloquei aqui a verificacao se tem descontos/acrtescimos
    lancou_acrescimo := auxDataset.FieldByName('despesasacessorias').AsFloat;
    lancou_desconto := auxDataset.FieldByName('desconto').AsFloat;

    //06/11/2003 - Por Helder Frederico: isso estava na LancaCupomFiscal
    // Coloquei aki para melhorar a performance, com o mínimo de SQL!!
    GravaINI(TIPO_TERMINAL, 'valorultimocupom',auxdataset.FieldByName('totalpedido').AsString);
    ...
    with auxDataset do
        dmorc.RunSQL('INSERT INTO PDV_cabecalhocupom ' +
            ...
            'VALUES ' +
            '(0, 1,' +
            FieldByName('cli_codcliente').AsString + ', ' +
            ...
            {floattostr(strtofloatdef(FieldByName('numpessoas').AsString,0))}
            //dia 06/11/2003 por Helder Frederico: Força gravar o numero de pessoas em 1
            '1' + ',0 )');
    DecimalSeparator := ',';
    ...
    begin
        ...
        begin
            dmORC.RunSQL('INSERT INTO PDV_DETALHECUPOM ' +
                '( SITUACAO, CFG_CODCONFIG, CLI_CODCLIENTE, VEN_CODVENDEDOR, ' +
                'IDUSUARIO, DATAHORA, NCP, TERMINAL, CODIGOPRODUTO,' +
                'SEQ, VALORUN, QTDE, DESCONTO, ACRESCIMO,SITUACAO) ' +
                'VALUES ' +
                '(0, 1,1, ' +
...
begin
    ...
    sql_txt:='INSERT INTO PDV_FECHAMENTOCUPOM  ' +
        ...
        ' VALORRECEBIDO, BANCO, NUMEROCHEQUE, AGENCIA, CONTA, DATAPRE, oid)' +
        ' VALUES ' +
        '(' +
        IntToStr(leini('terminal', 'PDVNum')) + ', ' +
        NCP + ', ' +
...
// dia 15/12/2003 - por Helder Frederico: estava colocando o desconto negativo
        'desconto = desconto   + (' + quotedstr(FloatToStr((auxDesconto /
        auxQtdItens))) + ')*QTDE ' +
        'WHERE (datahora between  ' + QuotedStr(FormatDateTime('mm/dd/yyyy',
        DataMovimento)) +

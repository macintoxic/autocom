...
begin
    ...
    begin
         ...
         DMORC.Commit;
         dmORC.RunSQL('SELECT GEN_ID(SEQ_PESSOA,0) AS PES_CODPESSOA FROM PESSOA WHERE PES_NOME_A = '  + QuotedStr(mskNome.TEXT) + 'AND TELEFONE1 = ' + QuotedStr(mskFone.Text), auxdataset);
         DMORC.Commit;
         ...
         DMORC.RunSQL( 'INSERT INTO ENDERECOPESSOA ' +
                       ...
                       'VALUES' +
                       '( ' + IntToStr(j) +', '+
                             inttostr(strtointdef(LEINI(TIPO_TERMINAL,'TIPOENDERECOENTREGA'),2)) +', '+
...
var
   auxdataset:TDataSet;
begin
     mskCodigo.Enabled  := Boolean(IfThen(StrToIntDef(LeINI(TIPO_TERMINAL,'contadorcliente'),0) = 0, 0,1));
     if not mskCodigo.Enabled then
     begin
          dmorc.RunSQL('select distinct (GEN_ID(SEQ_Cliente, 0) + 1) as valor from cliente',auxdataset);

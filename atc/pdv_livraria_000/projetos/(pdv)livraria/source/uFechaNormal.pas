...
uses
    ...
    Inifiles, ExtCtrls, ComCtrls, StdCtrls, Mask, Buttons, db;
...
//    PesoBruto := 0;
    ...
    Vendedor := auxdataset.FieldByName('ven_codvendedor').Value;
    TOtalPedidoVenda := auxdataset.FieldByName('TotalPedido').Value;
    Cliente := auxdataset.FieldByName('Cli_codcliente').Value;
    NumPessoas := floattostr(strtofloatdef(auxdataset.FieldByName('NumPessoas').AsString, 1));
...
{    auxdataset := dmORC.ConsultaDetalhePedido(NumeroPedido);
    ...
    begin
        pesoLiquido := pesoLiquido +
            auxdataset.FieldByName('pesoliquido').Value;
        PesoBruto := PesoBruto + auxdataset.FieldByName('pesoliquido').Value;
        auxdataset.Next;
    end;
    freeandnil(auxdataset);}
end;
    ...
    begin
        ...
        begin
            ...
            begin
                ...
                begin
                    strAux := cnfv(StrToInt(leini('terminal', 'ModECF')),
                        StrToInt(leini('terminal', 'comECF')));
                    ...
                    begin
                        ...
                        comprovante.Add('OPERADOR: ' + LeINI('OPER', 'CODIGO',
                            'dados\oper.ini') + ' ' + LeINI('OPER', 'NOME',
                            'dados\oper.ini'));
            ...
            else// dia 06/11/2003 por Helder Frederico: Muda a situacao do pedido para X (recebido)
               begin
                   dmORC.RunSQL('UPDATE PEDIDOVENDA SET SITUACAO='+QUOTEDSTR('X')+' WHERE NUMEROPEDIDO = '+Pedido);
                   dmORC.Commit;
               end;

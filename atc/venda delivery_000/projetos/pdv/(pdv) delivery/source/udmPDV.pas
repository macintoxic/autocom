...
begin
    ...
    if auxDataSet.IsEmpty then
        RunSQL(
            ...
            QuotedStr(floattostr(StrToFloat(DESPESASACESSORIAS))) + ',' +
        ...
        RunSQL('UPDATE PEDIDOVENDA SET ' +
            ...
            ' DESPESASACESSORIAS      = ' + QuotedStr(DESPESASACESSORIAS) + ','
                +
...
begin
    RunSQL('select numeropedido Pedido, cliente.codigocliente Codigo, pes_nome_a Nome, pedidovenda.situacao from pedidovenda '
        +
        'inner join cliente on (cliente.cli_codcliente = pedidovenda.cli_codcliente) ' +
        'inner join pessoa  on (cliente.pes_codpessoa = pessoa.pes_codpessoa) ' +
        'where ' +

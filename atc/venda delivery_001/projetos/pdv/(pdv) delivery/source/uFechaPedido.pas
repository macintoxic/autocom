...
begin
     ...
     mskDesconto.Text  := FormatFloat('0.00', StrToFloat(auxdataset.FieldByName('DESCONTO').AsString));

     lblTotalCupom.Caption :=  auxdataset.FieldByName('totalpedido').AsString;
     FCodigoPedido := auxdataset.FieldByName('codigopedidovenda').AsString;
     FTotalPedido := auxdataset.FieldByName('TotalPedido').Value;

     mskAcrescimo.Text := FORMATFLOAT('0.00', auxdataset.FieldByName('DESPESASACESSORIAS').Value);
     ...
     begin
          pesoLiquido := pesoLiquido +  auxdataset.FieldByName('pesoliquido').Value;
...
begin
     ...
     lblTotalCupom.Caption := floattostr(FTotalPedido + StrToFloatDef(mskAcrescimo.Text,0) - StrToFloatDef(mskDesconto.Text,0))
end;
...
begin
  case Application.MessageBox('Confirma a efetivação do pedido?', TITULO_MENSAGEM,MB_YESNOCANCEL +MB_ICONQUESTION) of
        IDYES : begin
              dsFormasPagamento.GotoBookmark(lbFormaPagamentos.Selected.Data);
              auxConta := StrToFloat(lblTotalCupom.Caption) - StrToFloat(mskAcrescimo.Text)
                          + StrToFloat(mskDesconto.Text);
              dmORC.EnviaFaturamento( FPedido,dsFormasPagamento.FieldByName('CODIGOCONDICAOPAGAMENTO').AsString,
                                      FloatToStr(auxconta),
                                      memObs.Text,
                                      mskAcrescimo.Text,edOBSAcrescimo.Text,
                                      mskDesconto.Text,
                                      edOBSDesconto.Text);

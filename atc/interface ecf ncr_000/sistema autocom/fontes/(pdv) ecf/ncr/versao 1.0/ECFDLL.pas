...
begin
     AssignFile(LOGfile,extractfilepath(application.exename)+'_ncr1.LOG');
     if not fileexists(extractfilepath(application.exename)+'_ncr1.LOG') then Rewrite(logfile) else Reset(Logfile);
     Append(logfile);
...
=========================================================}
     ...
     espera_ecf(porta);
     try
        ncr:=TIniFile.Create(extractfilepath(application.exename)+'_ncr1.INI');
...
//{tipo para a NCR 23} -> 'NCR - IF-7140 Duas Estações v1.00'

     if (tipo=22) or (tipo=23)  then
        begin
...
{========================================================
  ...
  modal  -> descição das modalidades de pagamento
=========================================================}
     func:='Inicio do dia';
     ...
     _ncr1:=TIniFile.Create(extractfilepath(application.exename)+'_ncr1.ini');
     _ncr1.writeString('FINALIZADORAS', '01', m[1]);
     _ncr1.writeString('FINALIZADORAS', '02', m[2]);
     _ncr1.writeString('FINALIZADORAS', '03', m[3]);
     _ncr1.writeString('FINALIZADORAS', '04', m[4]);
     _ncr1.writeString('FINALIZADORAS', '05', m[5]);
     _ncr1.writeString('FINALIZADORAS', '06', m[6]);
     _ncr1.writeString('FINALIZADORAS', '07', m[7]);
     _ncr1.writeString('FINALIZADORAS', '08', m[8]);
     _ncr1.writeString('FINALIZADORAS', '09', m[9]);
     _ncr1.writeString('FINALIZADORAS', '10', m[10]);
     _ncr1.Free;
...
=========================================================}
     ...
     if strtofloat(trim(qtde))=1 then qtde:='1000';
...
=========================================================}
     ...
     _ncr1:=TIniFile.Create(extractfilepath(application.exename)+'_ncr1.ini');
     desc:=_ncr1.readString('FINALIZADORAS', moda, moda);
     _ncr1.Free;
     ...
     if moda<>'00' then comando:='25'+desc+'\'+valor;
     strpcopy(cmd,comando);
     log(func+' - Enviado (NCR):'+comando);
     enviacomando(porta,cmd);
     result:=Prepara_Resp(tipo,retorno,'','');
...
=========================================================}
     ...
     if nf='0' then
        ...
        begin
           func:='Inicio de operação não-fiscal';
           comando:='04X4';
           strpcopy(cmd,comando);
           log(func+' - Enviado (NCR):'+comando);
           enviacomando(porta,cmd);
...
=========================================================}
     func:='Venda Liquida';
     comando:='Comando_para_capturar_o_valor_da_venda_liquida';
     strpcopy(cmd,comando);
     log(func+' - Enviado (NCR):'+comando);
...
=========================================================}
     ...
     if c='' then //nulo é para somar 1 ao coo
        begin
           strpcopy(cmd,comando);
           log(func+' - Enviado (NCR):'+comando);
           enviacomando(porta,cmd);
           c:=floattostr(strtofloat(copy(retorno,20,6)));
...
=========================================================}
     ...
     _ncr1:=TIniFile.Create(extractfilepath(application.exename)+'_ncr1.ini');
     desc:=_ncr1.readString('FINALIZADORAS', modal, modal);
     _ncr1.Free;

     espera_ecf(porta);
     comando:='190\Sangria - Finalizadora : '+desc+strtovalor(2,trim(valor));
     strpcopy(cmd,comando);
     log(func+' - Enviado (NCR):'+comando);
     enviacomando(porta,cmd);
     result:=Prepara_Resp(tipo,retorno,'','');
...
//{tipo para a NCR 23} -> 'NCR - Duas Estações v1.00'

     if tipo=23 then ok:=true else ok:=false;
...
var d,c,e,f,g,h,i,j,k,l,m,n:string;
...
=========================================================}
        ...
        begin
           ...
           if copy(retorno,1,2)<>'.-' then // se vier um '.-' , significa q ocorreu problema!!
              begin
                 ...
                 log(func+' - Enviado (NCR):'+comando);
                 enviacomando(porta,cmd);
                 if length(retorno)<45 then retorno:='';
                 delete(retorno,1,6);
                 r[1]:=copy(retorno,1,12); // gt atual;
                 delete(retorno,1,pos('\',retorno));
                 r[2]:=copy(retorno,1,pos('\',retorno)-1); // coo atual;
                 delete(retorno,1,pos('\',retorno));
                 r[3]:=copy(retorno,1,pos('\',retorno)-1); // numero de documentos cancelados;
                 delete(retorno,1,pos('\',retorno));
                 r[4]:=copy(retorno,1,pos('\',retorno)-1); // numero de op. não sujeitas ao icms;
                 delete(retorno,1,pos('\',retorno));
                 r[5]:=copy(retorno,1,pos('\',retorno)-1); // CRO;
                 delete(retorno,1,pos('\',retorno));
                 r[6]:=copy(retorno,5,12); // gt inical;
                 delete(retorno,1,pos('\',retorno));
                 r[7]:=copy(retorno,1,pos('\',retorno)-1); // coo inicial;
                 delete(retorno,1,pos('\',retorno));
                 r[8]:=copy(retorno,1,pos('\',retorno)-1); // numero de doc. cancelados inicial
                 delete(retorno,1,pos('\',retorno));
                 r[9]:=copy(retorno,1,pos('\',retorno)-1); // numero de op. não sujeitas ao icms inicial;
                 delete(retorno,1,pos('\',retorno));
                 r[10]:=copy(retorno,1,pos('\',retorno)-1); // cro inicial
                 delete(retorno,1,pos('\',retorno));

                 try
                    i:=enche(floattostr(strtofloat(r[7])),'0',1,6); // coo inicial
                 except
                    i:=enche('0','0',1,6);
                 end;

                 try
                    j:=enche(floattostr(strtofloat(r[2])),'0',1,6); // coo  final
                 except
                    j:=enche('0','0',1,6);
                 end;

                 try
                    l:=enche(floattostr(strtofloat(r[6])),'0',1,12); // gt inicial
                 except
                    l:=enche('0','0',1,12);
                 end;

                 try
                    m:=enche(floattostr(strtofloat(r[1])),'0',1,12); // gt final
                 except
                    m:=enche('0','0',1,12);
                 end;
 ...
 {    try
        ncr:=TIniFile.Create(extractfilepath(application.exename)+'_ncr1.INI');
        ncr.writeString('IMPOSTO', 'T01', '0');
        ncr.writeString('IMPOSTO', 'T02', '0');
        ncr.writeString('IMPOSTO', 'T03', '0');
        ncr.writeString('IMPOSTO', 'T04', '0');
        ncr.writeString('IMPOSTO', 'T05', '0');
        ncr.writeString('IMPOSTO', 'T06', '0');
        ncr.writeString('IMPOSTO', 'T07', '0');
        ncr.writeString('IMPOSTO', 'T08', '0');
        ncr.writeString('IMPOSTO', 'T09', '0');
        ncr.writeString('IMPOSTO', 'T10', '0');
        ncr.writeString('IMPOSTO', 'T11', '0');
        ncr.writeString('IMPOSTO', 'T12', '0');
        ncr.writeString('IMPOSTO', 'T13', '0');
        ncr.writeString('IMPOSTO', 'T14', '0');
        ncr.writeString('IMPOSTO', 'T15', '0');
     finally
        ncr.Free;
     end;
}

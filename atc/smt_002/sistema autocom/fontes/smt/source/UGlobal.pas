...
type
  ...
  procedure EnableFields(Status: Boolean; ParentComponent: TControl; Form: TForm);
    ...
    begin
      if Status then
        begin
          ...
          if (Form.Components[i] as TControl).Parent = ParentComponent then
            (Form.Components[i] as TControl).Enabled := True;
        end
      else
        begin
          if (Form.Components[i] is TControl) then
            if (Form.Components[i] as TControl).Parent = ParentComponent then
              begin
                (Form.Components[i] as TControl).Enabled := False;

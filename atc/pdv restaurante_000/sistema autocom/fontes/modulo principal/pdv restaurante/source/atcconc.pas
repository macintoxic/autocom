...
const
     ...
     Nome_sistema_cabecalho='AutocomPLUS - Cardápio - Restaurante PDV';
...
type
  Tfrm_FATCPR = class(TForm)
    ...
    Panel12: TPanel;
    Panel13: TPanel;
    mi206: TsuiImageButton;
    mi205: TsuiImageButton;
    mi801: TsuiImageButton;
    mi802: TsuiImageButton;
    mtrocaoperador: TsuiImageButton;
    msair: TsuiImageButton;
    Panel14: TPanel;
    Panel15: TPanel;
    mi109: TsuiImageButton;
    ...
    procedure mi206Click(Sender: TObject);
    procedure mtrocaoperadorClick(Sender: TObject);
...
// Monta a estrutura do menu conforme configuração no banco de dados
     ...
     Begin
        If frm_FATCPR.Components[a] is TsuiImageButton then
        Begin
           menu := frm_FATCPR.Components[a] as TsuiImageButton;
...
procedure Tfrm_FATCPR.mi206Click(Sender: TObject);
var
   Menu:TsuiImageButton;
   s_MenuNome:string;
begin
     menu:= sender as TsuiImageButton;
     tbl_permissaosistema.close;
     tbl_permissaosistema.sql.clear;
     tbl_permissaosistema.sql.add('select * '+
                                  'from permissaosistema '+
...
procedure Tfrm_FATCPR.mtrocaoperadorClick(Sender: TObject);
var m:integer;
    menu:TsuiImageButton;
begin
     frm_login.showmodal;
...
procedure Tfrm_FATCPR.msairClick(Sender: TObject);
begin
     close;
end;

procedure Tfrm_FATCPR.FormKeyDown(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
     if (key=vk_f1) and (mi206.Enabled=true)          then mi206Click(mi206);
     if (key=vk_f2) and (mi205.Enabled=true)          then mi206Click(mi205);
     if (key=vk_f3) and (mi801.Enabled=true)          then mi206Click(mi801);
     if (key=vk_f4) and (mi802.Enabled=true)          then mi206Click(mi802);
     if (key=vk_f5) and (mi109.Enabled=true)          then mi206Click(mi109);
     if (key=vk_f6) and (mtrocaoperador.Enabled=true) then mtrocaoperadorClick(mtrocaoperador);
     if (key=vk_f7) and (msair.Enabled=true)          then msairClick(msair);
end;

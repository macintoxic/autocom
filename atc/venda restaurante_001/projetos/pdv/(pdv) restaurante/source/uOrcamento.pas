...
type
    TfrmPDV = class(TForm)
        ...
        cdsItensPedidoMotivoCancelamento: TStringField;
        ...
        procedure lvMesasMouseDown(Sender: TObject; Button: TMouseButton;
            Shift: TShiftState; X, Y: Integer);
        procedure FormActivate(Sender: TObject);
        procedure tabMesasShow(Sender: TObject);
        procedure lvMesasKeyDown(Sender: TObject; var Key: Word;
            Shift: TShiftState);
...
uses StrUtils, udmPDV, Math, uFechaNormal,
    ...
    uDadosChequeLeitor, uQrFechamento, uInforme;
    ...
    begin
        ...
        begin
            ...
            begin
                ...
                cdsItensPedidoMotivoCancelamento.value :=
                    auxdataset.FieldByName('Motivo_cancelamento').AsString;
        ...
        if dbgrdItens.SelectedRows.Count = 0 then
            ...
            begin
                    ...
                    begin
                         ...
                         if (frmMotivo.ShowModal = mrcancel) then
                         begin
                              FreeAndNil(frmMotivo);
                              Exit;
                         end
                            else
                             if (trim(frmMotivo.edMotivo.Text) = '') then
                               begin
                                    Application.MessageBox('O motivo do cancelamento é OBRIGATÓRIO.',TITULO_MENSAGEM, MB_OK + MB_ICONWARNING);
                                    FreeAndNil(frmMotivo);
                                    Exit;
                               end;
                           dbgrdItens.DataSource.DataSet.Edit;
                           dbgrdItens.DataSource.DataSet.FieldByName('MotivoCancelamento').Value := 'Comanda: ' + frmMotivo.edComanda.Text + ' Motivo: ' + frmMotivo.edMotivo.Text;
                           FreeAndNil(frmMotivo);
                    end;
    ...
    begin
        ...
        begin
            ...
            if strtointdef(leini('terminal', 'printgrill'), 0) = 1 then
                printgrill(StrToInt(mskControle.Text), leini(TIPO_TERMINAL,
                    'legenda'),
                    mskIndicador.Text);
...
begin
    ...
    begin
        ...
        begin
            ...
            auxDataSet := dmORC.ConsultaCliente('1');
            if not auxDataSet.IsEmpty then
            begin
                auxDataSet.First;
            end;
...
procedure TfrmPDV.lvMesasMouseDown(Sender: TObject; Button: TMouseButton;
    Shift: TShiftState; X, Y: Integer);
begin
    if (mskcontrole.Enabled) and (lvMesas.GetItemAt(x, y) <> nil) then
        if Button = mbLeft then
        begin
            mskControle.Text := lvMesas.GetItemAt(x, y).Caption;
            mskControleExit(mskControle);
        end
end;
...
begin
    lvMesas.Items.Clear;
    auxdataset :=
        dmORC.BuscaListaPedido(inttostr(strtointdef(LEINI(TIPO_TERMINAL,
        'codigo_origem'), 2)));
    auxdataset.Last;
    auxdataset.First;
    while not auxdataset.Eof do
    begin
        with lvMesas.Items.Add do
        begin
            Caption := auxdataset.fieldbyname('Pedido').AsString;
            if auxdataset.fieldbyname('Situacao').AsString = 'Z' then
                ImageIndex := 0
            else
                ImageIndex := 1;
...
procedure TfrmPDV.lvMesasKeyDown(Sender: TObject; var Key: Word;
    Shift: TShiftState);
begin
    if key = vk_return then
        if lvMesas.ItemFocused <> nil then
        begin
            mskControle.Text := lvMesas.ItemFocused.Caption;
            mskControleExit(mskControle);
        end;
end;

...
uses
    SysUtils, Classes, DB, IBDatabase, IBQuery, IBSQL, IBCustomDataSet;
...
type
    ...
    public
        ...
        function PegaCodigoInternoCliente(strCodigo:string):string;
    end;
...
procedure TdmORC.C(Sender: TObject);
begin
    IBDatabase.Close;
...
begin
    ...
    if auxDataSet.IsEmpty then
        RunSQL(
            ...
            IfThen(StrToInt(DESCONTO) = 0, 'null',DESCONTO) + ',' +
        ...
        RunSQL('UPDATE PEDIDOVENDA SET ' +
            ...
            ' DESCONTO                = ' + IfThen(StrToInt(DESCONTO) = 0, 'null',DESCONTO) + ',' +
    ...
    begin
        RunSQL(' INSERT INTO PRODUTOPEDIDOVENDA ' +
            ' (CODIGOPRODUTO, QUANTIDADE, PRECO, CODIGOPEDIDOVENDA, OBSERVACAO, IMPRESSO, CANCELADO) ' +
            ' VALUES (' +
            ...
            + ', ' +
            strAuxCodigoPedidoVenda + ', ' +
            ifthen(dsItens.FieldByName('Observacao').AsString <> '', dsItens.FieldByName('Observacao').AsString, 'NULL') + ', ' +
            ifthen(dsItens.FieldByName('Impresso').AsString   <> '', dsItens.FieldByName('Impresso').AsString,   'NULL') + ', ' +
            ifthen(dsItens.FieldByName('situacao').AsString   <> '', dsItens.FieldByName('situacao').AsString,   'NULL') + ')');
        dsItens.Next;
    end;
...
begin
    RunSQL('select numeropedido Pedido, vendedor.codigovendedor Codigo, pes_nome_a COMPLETO, pedidovenda.situacao from pedidovenda '
        +
...
function TdmORC.PegaCodigoInternoCliente(strCodigo: string): string;
var
   auxDataSet:TDataSet;
begin
     RunSQL('SELECT CLI_CODCLIENTE FROM CLIENTE WHERE CODIGOCLIENTE = ' + strCodigo, auxDataSet);
     Result := auxDataSet.Fields[0].AsString;
     FreeAndNil(auxDataSet);
end;
